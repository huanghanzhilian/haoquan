      ...
      <section class="cart_food_list " v-show="showDialogue">
        <div class="opslo_wrap" ref="input">
          ...
          <ul class="opslo_list_wrap">
            <li class="opslo_list" v-for="item in options" @click="increase(item.a,item.title)">
              <div class="item_left">{{item.title}}
                <span class="text_dep">{{item.time}}</span>
              </div>
              <div class="item_right">
                <i v-if="item.a==value.a" class="iconfont icon-duigou- icon_styles"></i>
              </div>
            </li>
          </ul>
        </div>
